...
comparacion_final <- resultados_analiticos %>%
  ...
  mutate(
    ...
    Var_Simulada    = Var_Simulada
  )

comparacion_final

#Se observa que los valores son casi ide´neticos para los 4 parámetros. La variabilidad onbservada podría deberse al muestreo aleatorio que se hace cuando se extraen las 500 submuestras para la aproximación de MC.
...
# Calculamos MCSE y el IC para cada clase
ic_mc <- tibble(
  Clase = c("Clase 1", "Clase 2"),
  Media_Simulada = c(mean(muestras_clase_1), mean(muestras_clase_2)),
...
# Redondeamos para mostrar mejor
ic_mc %>%
...
# Usamos la varianza simulada mayor entre ambas clases y calculamos el tamaño muestral necesario para que MCSE <= 0.005 (esto evita que se afecte la cifra anterior en el redondeo)
var_max <- max(var(muestras_clase_1), var(muestras_clase_2))
desv_max <- sqrt(var_max)

# Despejando n: MCSE = sd / sqrt(n) → n = (sd / MCSE)^2
mcse_deseado <- 0.005
n_requerido <- ceiling((desv_max / mcse_deseado)^2)

# Simulamos de nuevo usando este tamaño muestral
...
# Creamos un resumen comparativo entre los valores analíticos y los simulados con n_requerido

comparacion_precision <- tibble(
  Clase = c("Clase 1", "Clase 2"),
  
  # Valores analíticos (de la distribución posterior)
  Media_Analitica = c(mu_post_1, mu_post_2),
  Desv_Analitica  = c(sqrt(sigma2_post_1), sqrt(sigma2_post_2)),
  
  # Valores simulados con el tamaño muestral grande
  ...
  mutate(across(where(is.numeric), round, 4)) # Redondeamos para mejor presentación
...
credibilidad_sim <- tibble(
  Clase = c("Clase 1", "Clase 2"),
  IC99_Inf_Sim = c(quantile(muestras_clase_1_grande, 0.005),
                   quantile(muestras_clase_2_grande, 0.005)),
  IC99_Sup_Sim = c(quantile(muestras_clase_1_grande, 0.995),
...
credibilidad_ana <- tibble(
  Clase = c("Clase 1", "Clase 2"),
  IC99_Inf_Ana = qnorm(0.005, mean = c(mu_post_1, mu_post_2),
                              sd   = c(sqrt(sigma2_post_1), sqrt(sigma2_post_2))),
  IC99_Sup_Ana = qnorm(0.995, mean = c(mu_post_1, mu_post_2),
                              sd   = c(sqrt(sigma2_post_1), sqrt(sigma2_post_2)))
)
...
```{r}
# Parámetros
mu_D <- mu_post_2 - mu_post_1
sigma_D <- sqrt(sigma2_post_1 + sigma2_post_2)

# Probabilidad analítica
prob_analitica <- 1 - pnorm(0, mean = mu_D, sd = sigma_D)
...
```{r}
# Resultado original
prob_original <- mean(muestras_clase_2_grande > muestras_clase_1_grande)

# Aleatorizamos una de las muestras (por ejemplo, clase 1)
muestras_clase_1_shuffle <- sample(muestras_clase_1_grande)

# Probabilidad tras reordenar
...
# Estimamos el valor de la moda (maximo de la densidad)
...
```{r}
mu_post_1  # Estimador analítico de la media posterior para la Clase 1
mu_post_2  # Estimador analítico de la media posterior para la Clase 2
...
# Rango de valores para el eje horizontal (ajustado para el límite en 5)
x_range_sin <- seq(0, 5, by = 0.002)  # Limite del eje X en 5
x_range_con <- seq(0, 5, by = 0.002)  # Limite del eje X en 5

# Cálculo de las densidades para cada distribución Gamma
dens_sin <- dgamma(x_range_sin, shape = shape_sin, rate = rate_sin)
dens_con <- dgamma(x_range_con, shape = shape_con, rate = rate_con)

# Graficamos las dos distribuciones sobre el mismo gráfico con límite en el eje X
ggplot() +
  geom_line(aes(x = x_range_sin, y = dens_sin), color = PALETA[1], size = 1.2) +
  geom_line(aes(x = x_range_con, y = dens_con), color = PALETA[2], size = 1.2) +
  labs(
  ...
  scale_color_manual(values = c(PALETA[1], PALETA[2]), labels = c("Sin título universitario", "Con título universitario")) +
  xlim(0, 5) +  # Establecemos el límite del eje X en 5
  theme_minimal()
...
```{r}
# Número de simulaciones
n_sim <- 5000  # Este tamaño muestral se justifica para lograr precisión de tres decimales

# Simulación de las distribuciones Gamma
set.seed(20250318)
...
# Calcular la probabilidad de que la tasa de fertilidad sin título sea superior a la de con título
probabilidad <- mean(diferencias > 0)

# Calcular el intervalo de confianza del 99% para la diferencia de medias
ic_diferencia <- quantile(diferencias, probs = c(0.005, 0.995))

# Mostrar resultados
list(
  probabilidad = probabilidad,
  ic_diferencia = ic_diferencia
)
