...
```
# Cada valor p predictivo indica el % de simulaciones que tienen máximos menores que los datos empíricos. El valor especialmente alto del grupo con podría hablar de una discrepancia entre el modelo y lo observado.
...
# Se busca el valor p predictivo para una muestra de mujeres universitarias de tamaño `{r} n_con` en la que el nñumero de mujeres con 4 de hijos sea igual o menor que el máximo empírico.
...
conteo_menor_o_igual_con <- sum(mujeres_4_hijos_con_simuladas <= n_4_hijos_empirico_con)

# Calcular el valor-p
valor_p_con_menor <- conteo_menor_o_igual_con / length(mujeres_4_hijos_con_simuladas)
valor_p_con_menor



```
#El valor p predictovo indica que un 38,7% de las observaciones empíricas tiene un número de mujeres con 4 hijos mayor que los datos empíricos.

#El valor p predictivo moderado indica que es relativamente probable que el número de mujeres con 4 hijos observado en la muestra empírica pudiera haber ocurrido (no es muy extremo) bajo el modelo de distribución posterior.
:::
