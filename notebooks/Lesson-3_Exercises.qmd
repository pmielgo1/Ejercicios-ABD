...
# Graficar la PDF
ggplot(data = data.frame(x = x_vals, pdf = pdf_vals), aes(x = x, y = pdf)) +
...
#Se observa que, cuando alfa=beta, hay simetría; cuando alfa<beta, hay sesgo a la izquierda; y cuando alfa>beta, a la derecha.
...
#Cuando alfa=beta=ligeramente>1, se ve que la distribución empieza a tomar la forma acamapanada. Cuando solo uno de los parámtros es ligeramente>1, se observa que el sesgo de la función de exagera.
...
# Segunda muestra (ya definida, no sería realmente necesario repetirla aquí)
...
# Graficar todas las distribuciones (previa (en este comando), posterior 1ª muestra, posterior 2ª muestra)
