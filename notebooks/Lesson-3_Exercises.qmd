...
x_vals <- seq(mu_0 - 4 * sqrt(sigma_0_sq), mu_0 + 4 * sqrt(sigma_0_sq), length.out = 1000)

# Asegurarse que 'prior' y 'posterior' tengan la misma longitud que x_vals
prior <- dnorm(x_vals, mean = mu_0, sd = sqrt(sigma_0_sq))
posterior <- dnorm(x_vals, mean = mu_0, sd = sqrt(sigma_0_sq)) * prior

# Calcular el estimador esperado (media posterior) con integración numérica
PREC <- diff(x_vals)[1]  # Suponiendo que el paso entre x_vals es constante
estimador_esperado <- sum(x_vals * posterior * PREC) / sum(posterior * PREC)

# La moda será el máximo de la distribución posterior
