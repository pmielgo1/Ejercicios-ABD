...
#No, en la uniforme planteada, el 1 es el único valor posible (otra cosa sería que se redujese el rango, en cuyo caso, si el área no varía, podría ser mayor que 1). En cuanto a la normal, esta tendrá valores mayores que uno cuando la varianza adquiera valores muy bajos.
:::
...
# Agregar leyenda
legend("topleft", legend = c("Previo (Beta(1,1))", "Posterior 1ª Muestra", "Posterior 2ª Muestra"),
...
# Calcular el estimador esperado (media posterior) con integración numérica
estimador_esperado <- sum(x_vals * posterior * PREC) / sum(posterior * PREC)
estimador_esperado

#La moda será el máximo de la distribución
estimador_modal <- x_vals[which.max(posterior)]
estimador_modal

# Representación gráfica con los nuevos valores
...
abline(v = estimador_esperado, col = "green", lwd = 2, lty = 2)  # Media posterior
...
#Se observa una ligera discrepancia entre la media (que coincide con la solución analítica) y la moda, pero es tan pequeña que no se llega a apreciar en el gráfico.
```
::: 
