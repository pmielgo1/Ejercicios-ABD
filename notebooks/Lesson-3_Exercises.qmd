...
::: #Para la distribución uniforme, el máximo está en cualquier valor y es simepre 1:
...
#En este caso, el valor máximo que alcanza la distribución es 0,399 (aprox)
...
::: #En ambos casos, el valor de frencuencia máxima es la moda. En el caso de la normal. En el caso de la uniforme, todo el rango tiene el mismo valor, así que no hay una moda estricta como tal. Para la normal, el valor de la moda y el de la media coinciden, por lo que tanto media como moda serán x=0.
...
::: No, en la uniforme planteada, el 1 es el único valor posible (otra cosa sería que se redujese el rango, en cuyo caso, si el área no varía, podría ser mayor que 1). En cuanto a la normal, no puede ser mayor que 1 en tanto que la normal viene definida como 1/(sqrt(2*pi*s)*e((x-mu)^2)/(2*s)^2)), es decir, el valor teórico máximo será siempre 1.
...
normal <- tibble(
  variable = seq(-4, 4, by = 1e-3),
  cdf = pnorm(variable, mean = 0, sd = 1)  # pnorm ya da la CDF
)

# Se grafica la CDF

ggplot(normal, aes(x = variable, y = cdf)) +
  geom_line(color = "blue") +
  labs(title = "Función de Distribución Acumulativa (CDF)",
       x = "Valor",
...
mean_normal <- integrate(function(x) x * dnorm(x), -Inf, Inf)$value
mean_normal

#También se puede calcular la cdf asociada a E:
...
::: #Esta función es no informativa, ya que es uniform en [0,1].
...
# Crear los datos con nuevos valores de α y β
data_beta <- tibble(
...
::: #Se observa que, cuando a y b son pequeños, la curva acumula densidad en los extremos, mientras que cuando crecen lo hacen el centro, asemejándose a una normal como indica el TCL. Cuando a y B son 1, la curva es una uniforme.
...
# Crear los datos con diferentes valores de α y β
data_beta <- tibble(
  ...
  y = c(dbeta(seq(0, 1, by = 1e-3), 1, 1),   # Uniforme
        ...
        dbeta(seq(0, 1, by = 1e-3), 2, 5),   # Sesgo a la izquierda
        dbeta(seq(0, 1, by = 1e-3), 5, 2)),  # Sesgo a la derecha
  curva = rep(c("α=1, β=1 (Uniforme)", 
                "α=5, β=5 (Simétrica centrada (similar a normal))", 
                "α=0.5, β=0.5 (Densidad en extremos)", 
                "α=2, β=5 (Sesgo a la izquierda)", 
                "α=5, β=2 (Sesgo a la derecha)"), 
...
#Se observa que, cuando a=B, hay simetría; cuando a<B, hay sesgo a la izquierda; y cuando a>B, a la derecha.
...
data_beta <- tibble(
  ...
  y = c(dbeta(seq(0, 1, by = 1e-3), 1.05, 1.05),  # Casi uniforme
        dbeta(seq(0, 1, by = 1e-3), 2, 1.05),    # Sesgo a la derecha
        dbeta(seq(0, 1, by = 1e-3), 1.05, 2)),   # Sesgo a la izquierda
  curva = rep(c("α=1.05, β=1.05 (Casi uniforme)", 
                "α=2, β=1.05 (Sesgo a la derecha)", 
                "α=1.05, β=2 (Sesgo a la izquierda)"), 
              each = length(seq(0, 1, by = 1e-3)))
)
...
#Cuando a=B=ligeramente>1, se ve que la distribución empieza a tomar la forma acamapanada. Cuando solo uno de los parámtros es ligeramente>1, se observa que el sesgo de la función de exagera.
...
# Crear los datos con valores de α y β ligeramente y moderadamente por debajo de 1
data_beta <- tibble(
  x = rep(seq(0, 1, by = 1e-3), 4),
  y = c(dbeta(seq(0, 1, by = 1e-3), 0.9, 0.9),   # Forma en "U"
        dbeta(seq(0, 1, by = 1e-3), 0.5, 0.5),   # Forma en "U" más pronunciada
        dbeta(seq(0, 1, by = 1e-3), 0.8, 0.5),   # Sesgo a la izquierda
        dbeta(seq(0, 1, by = 1e-3), 0.5, 0.8)),  # Sesgo a la derecha
  curva = rep(c("α=0.9, β=0.9 (Forma en U)", 
                "α=0.5, β=0.5 (Forma en U más pronunciada)", 
                "α=0.8, β=0.5 (Sesgo a la izquierda)", 
                "α=0.5, β=0.8 (Sesgo a la derecha)"), 
              each = length(seq(0, 1, by = 1e-3)))
)
...
# La curva toma la forma de U. La dirección del sesgo cuando a=/B se invierte respecto a los valores >1.


# Crear los datos con un α mayor que 1 y un β menor que 1, y viceversa
data_beta <- tibble(
  x = rep(seq(0, 1, by = 1e-3), 2),
  y = c(dbeta(seq(0, 1, by = 1e-3), 2, 0.5),   # α > 1, β < 1 (Sesgo a la derecha)
        dbeta(seq(0, 1, by = 1e-3), 0.5, 2)),   # α < 1, β > 1 (Sesgo a la izquierda)
  curva = rep(c("α=2, β=0.5 (Sesgo a la derecha)", 
                "α=0.5, β=2 (Sesgo a la izquierda)"), 
              each = length(seq(0, 1, by = 1e-3)))
)
...
# Datos de la muestra
n <- 22
y <- sum(aceptacion_muestra$resp_descarga_app == "Si")  # Contamos los "Sí"
...
expected_value <- alpha_posterior / (alpha_posterior + beta_posterior)

# Moda de la distribución posterior (siempre que alpha > 1 y beta > 1, por la forma de la distribución)

if (alpha_posterior > 1 && beta_posterior > 1) {
  mode_value <- (alpha_posterior - 1) / (alpha_posterior + beta_posterior - 2)
} else {
  # Si no se puede calcular la moda, la moda será un valor en los extremos
  mode_value <- ifelse(alpha_posterior > beta_posterior, 1, 0)
}

# Mostrar resultados
expected_value
#Muestra el valor más probable de theta en la muestra.
...
# Segunda muestra (ya definida, no es necesario repetirla aquí)
...
# Ajustar la escala de los ejes para que se vea correctamente el gráfico
x_range <- seq(0, 1, length.out = 100)

# Graficar todas las distribuciones (previa, posterior 1ª muestra, posterior 2ª muestra)
plot(x_range, dbeta(x_range, 1, 1), type = "l", col = "gray", lwd = 2, 
     main = "Distribuciones Previa y Posteriores", 
     ylab = "Densidad", xlab = "Tasa de Aceptación", 
     xlim = c(0, 1), ylim = c(0, max(c(dbeta(x_range, 1, 1), dbeta(x_range, alpha_1, beta_1), dbeta(x_range, alpha_2, beta_2)))))
...
# Agregar leyenda
legend("topright", legend = c("Previo (Beta(1,1))", "Posterior 1ª Muestra", "Posterior 2ª Muestra"),
       col = c("gray", "red", "blue"), lwd = 2)

# Estimadores posterior (Esperado y Modal)
esperado <- alpha_2 / (alpha_2 + beta_2)

if (alpha_2 > 1 && beta_2 > 1) {
  modal <- (alpha_2 - 1) / (alpha_2 + beta_2 - 2)
} else {
  modal <- NA  # La moda no está definida si alguno de los parámetros es menor que 1
}
...
# Leyenda
legend("topright", legend = c("Previo", "Posterior 1ª Muestra", "Posterior 2ª Muestra", "Posterior Total"),
...
sigma_sq <- sigma_0_sq  # Usamos la varianza de la población

# Cálculo de la media y varianza de la distribución posterior
# Media posterior
mu_post <- (mu_0 / sigma_0_sq + mean_clase_1 * n_clase_1 / sigma_sq) / 
           (1 / sigma_0_sq + n_clase_1 / sigma_sq)

# Varianza posterior
sigma_post_sq <- 1 / (1 / sigma_0_sq + n_clase_1 / sigma_sq)
...
estimador_esperado <- mu_post  # La media posterior es el estimador esperado
estimador_modal <- mu_post  # La moda de la normal también es la media posterior

# Ajuste de los ejes y representación gráfica de las distribuciones previa y posterior

# Definir el rango de los ejes (4 desviaciones típicas alrededor de la media previa)
x_vals <- seq(mu_0 - 4 * sqrt(sigma_0_sq), mu_0 + 4 * sqrt(sigma_0_sq), length.out = 100)

# Distribución previa (normal)
prior <- dnorm(x_vals, mean = mu_0, sd = sqrt(sigma_0_sq))

# Distribución posterior (normal)
posterior <- dnorm(x_vals, mean = mu_post, sd = sqrt(sigma_post_sq))
...
plot(x_vals, prior, type = "l", col = "blue", lwd = 2, 
     ...
     ylim = c(0, max(c(prior, posterior)) + 0.01))

# Añadir la distribución posterior al gráfico
lines(x_vals, posterior, col = "red", lwd = 2)

# Añadir líneas verticales para los estimadores esperado y modal
abline(v = estimador_esperado, col = "green", lwd = 2, lty = 2)  # Estimador esperado
abline(v = estimador_modal, col = "purple", lwd = 2, lty = 2)  # Estimador modal

# Añadir leyenda
legend("topright", legend = c("Distribución Previa", "Distribución Posterior", 
                              "Estimador Esperado", "Estimador Modal"),
       col = c("blue", "red", "green", "purple"), lwd = c(2, 2, 2, 2), lty = c(1, 1, 2, 2))

#Tanto el valor esperado como la moda coinciden en la distribución normal y con los valores calculados mediante la solución analítica.
